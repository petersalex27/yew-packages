...
var combinator_Y = Bind[test_named](f_).In(Apply[test_named](
...
func TestInstruction(t *testing.T) {
	...
	println(combinator_Y.Apply(_Bind(_Var("f"), _Var("x")).In(_Apply(eq10.MakeInstance(), _Var("x"), _Var("x"), _Apply(_Var("f"), _Var("x"))))))
...
func TestLaziness(t *testing.T) {
	...
	actual := combinator_Y.Apply(inFunc)
